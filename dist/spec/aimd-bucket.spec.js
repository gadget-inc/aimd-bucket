...
(0, vitest_1.describe)("AIMDBucket", () => {
    ...
    (0, vitest_1.describe)("edge cases and error handling", () => {
        ...
        (0, vitest_1.it)("should process pending requests when no events trigger refill (lockup bug fix)", async () => {
            bucket = new index_1.AIMDBucket({
            ...
            await vitest_1.vi.advanceTimersByTimeAsync(334); // ~1/3 second = 1 token at rate 3
            const token4 = await bucket.acquire();
            let stats = bucket.getStatistics();
            (0, vitest_1.expect)(stats.tokensIssued).toBe(4);
            (0, vitest_1.expect)(stats.currentRate).toBe(3);
            token1.success();
            token2.success();
            token3.success();
            token4.success();
            ...
            await vitest_1.vi.advanceTimersByTimeAsync(5000); // 5 seconds = 15 tokens should be available
            stats = bucket.getStatistics();
            (0, vitest_1.expect)(stats.pendingCount).toBe(0);
            (0, vitest_1.expect)(stats.tokensIssued).toBe(9);
            const resolvedTokens = await Promise.all(pendingPromises);
            (0, vitest_1.expect)(resolvedTokens).toHaveLength(5);
        });
        (0, vitest_1.it)("should process pending requests with fractional rates", async () => {
            ...
            (0, vitest_1.expect)(firstToken).toBeInstanceOf(index_1.AIMDBucketToken);
            ...
            await vitest_1.vi.advanceTimersByTimeAsync(50);
            let stats = bucket.getStatistics();
            (0, vitest_1.expect)(stats.currentRate).toBe(0.5);
        ...
        (0, vitest_1.it)("should process pending requests with very low fractional rates", async () => {
            ...
            await vitest_1.vi.advanceTimersByTimeAsync(50);
            let stats = bucket.getStatistics();
            (0, vitest_1.expect)(stats.currentRate).toBe(0.1);
            ...
            await vitest_1.vi.advanceTimersByTimeAsync(20000);
